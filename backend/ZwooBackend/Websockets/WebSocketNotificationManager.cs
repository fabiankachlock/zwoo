 ...
 * Route:
 * WebSocketController
 * -> IWebSocketManager.AddConnection
 * -> [GameLogic].IPlayerManager.AddPlayer
 * -> await IWebSocketHandler.Handle(ws, shouldClose)
 * -> [GameLogic].IPlayerManager.RemovePlayer
 * -> IWebSocketManager.RemoveConnection
 * 
 * [WebSocket.NewMessage]
 * -> ZRPDecoder
 * -> [GameLogic].EventDistributer.Distribute()
 * 
 * [GameLogic.NewEvent]
 * #[onInit] ZRPMessageAdapter -> [GameLogic].EventDistributer.SetAdapter()
 * -> ZRPEncoder
 * -> IWebSocketNotificationDispatcher
 *    | -> IWebSocketManager.[SendPlayer|BrodcastGame|...]
 */

// TODO: this GameEvent to ZRP tranblations needs to move to gamelogic message dristributer
