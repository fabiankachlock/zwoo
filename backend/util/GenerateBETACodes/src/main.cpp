...
int main( )
{
    ...
    {
        s_code_amount.erase(
            std::remove_if( s_code_amount.begin( ), s_code_amount.end( ),
                            []( auto i ) { return !std::isdigit( i ); } ),
            s_code_amount.end( ) );
    };
    ...
    _port.on_change = [ & ]
    {
        s_db_port.erase( std::remove_if( s_db_port.begin( ), s_db_port.end( ),
                                         []( auto i )
    ...
    auto i_db_login_password =
        ftxui::Input( &s_db_login_password, "password", _pw );
    auto i_db_port = ftxui::Input( &s_db_port, "port", _port );
    auto i_save_to_file = ftxui::Checkbox( "", &save_to_file );
    auto i_generate_codes = ftxui::Button(
        "Generate",
        [ & ]( )
        {
            ...
            {
                codes =
                    generate_codes( std::stoi( s_code_amount ), s_code_format );
                s_codes = "";
                for ( auto i : codes )
                {
                    s_codes += i;
    ...
    auto i_save_to_db = ftxui::Button(
        "Save To DB",
        [ & ]( )
        {
    ...
    auto inputs = ftxui::Container::Vertical(
        { i_code_amount, i_code_format, i_db_login_name, i_db_login_password,
          i_db_port, i_save_to_file, i_generate_codes, i_save_to_db } );
    ...
    // Define Layout
    auto main_window = ftxui::Renderer(
        inputs,
        [ & ]
        {
            return ftxui::hbox(
                       { /* Sidebar */
                         ftxui::vbox(
                             { ftxui::text( "ZWOO Beta codes" ) | ftxui::bold,
                               ftxui::separator( ),
                               ftxui::hbox( ftxui::text( "amount: " ),
                                            i_code_amount->Render( ) ),
                               ftxui::separator( ),
                               ftxui::hbox( ftxui::text( "format: " ),
                                            i_code_format->Render( ) ),
                               ...
                               ftxui::hbox( ftxui::text( "name    : " ),
                                            i_db_login_name->Render( ) ),
                               ftxui::hbox( ftxui::text( "password: " ),
                                            i_db_login_password->Render( ) ),
                               ftxui::hbox( ftxui::text( "port    : " ),
                                            i_db_port->Render( ) ),
                               ftxui::separator( ),
                               ftxui::text( "General" ),
                               ftxui::hbox( ftxui::text( "save to file: " ),
                                            i_save_to_file->Render( ) ),
                               ftxui::separator( ),
