...
static auto random_char() -> std::string
{
    static const char alphanum[] = "0123456789"
                                   ...
                                   "abcdefghijklmnopqrstuvwxyz";
    return std::string( 1, alphanum[ random_number( 0, 512 ) %
                        ( sizeof( alphanum ) - 1 ) ]);
}
...
{
    return std::string( 1, (char)(random_number( 0, 25 ) + (32 * random_number( 0, all_chars))) + 65 );
}

static auto random_number( int min, int max) -> int
{
    auto r = std::mt19937(std::chrono::steady_clock::now( ).time_since_epoch( ).count( ));
