...
void ZwooInstanceListener::onAfterCreate(
    ...
    const std::shared_ptr<const ParameterMap> &params )
{
    ...
    auto wins = (uint32_t)stoi( params->find( "wins" )->second.getValue( "" ) );
    auto role = (uint8_t)stoi( params->find( "role" )->second.getValue( "" ) );
    auto username = params->find( "username" )->second.getValue( "" );
    ListenerData data = { guid, puid, role, username, wins };

    logger->log->info( "name: {3}, guid: {0}, puid: {1}, role: {2} Connected!",
                       guid, puid, role, username );
...
void ZwooInstanceListener::onBeforeDestroy(
    const oatpp::websocket::WebSocket &socket )
{
    SOCKETS--;
    auto listener =
        std::static_pointer_cast<ZwooListener>( socket.getListener( ) );
