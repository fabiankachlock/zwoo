...
CardRule::CardRule(uint8_t _ID, e_action _action, Card _cardtype, uint8_t _mod) : ID(_ID), action(_action), cardtype(_cardtype), mod(_mod) {}
...
uint8_t GameRules::createRule(int _type, uint16_t _arg1, uint16_t _arg2, uint16_t _arg3, uint16_t _arg4) {
	switch (_type) {
		case 1: // GameRule
			break;
		case 2: // CardRule
			std::cout << "CREATING A CARDRULE WITH FOLLOWING ARGUMENTS: " << _arg1 << ", " << _arg2 << ", " << _arg3 << ", " << _arg4 << "\n";
			uint8_t id = idcnt++;
			e_action action = static_cast<e_action>(_arg1);
			Card cardtype = Card(static_cast<e_color>(_arg2), static_cast<e_number>(_arg3));
...
bool GameRules::canPlace(Card _stacktopcard, Card _playerplacecard, bool _playerDraw) {	
	...
	if (_playerDraw) {
		...
		if (_stacktopcard.color == _playerplacecard.color || _stacktopcard.number == _playerplacecard.number) {
			return true;
		}
		else {
			std::cout << "WHAT THE HECK ARE YOU DOING?!? YOU CANT PLACE THIS CARD";
			return false;
		}
...
CardRule GameRules::getCardRule(Card _card) {
	for (auto it = cardrules.begin(); it != cardrules.end(); it++) {
		if (it->second->cardtype == _card) { // if card matches one card definition in cardrules
