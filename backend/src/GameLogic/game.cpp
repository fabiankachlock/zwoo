...
Game::Game() : ID(1), rules(GameRules()), pile(CardPile()), playerorder(), 
draw_active(false), current_player(playerorder.first()), current_draw_amount(0), active(false),
direction(1), pick_active(false), turncount(0) {
...
void Game::placeCard(std::shared_ptr<Player> _player, Card _card1, Card _card2) {
	std::shared_ptr<Card> card = _player->removeCard(_card1); // remove card from player inventory
...
bool Game::isValidAction(e_gaction _action) {
	...
	for (auto it = expected_actions.begin(); it != expected_actions.end(); it++) {
		if (_action == *it) {
			return true;
		}
	}
...
std::vector<e_gaction> Game::getExpectedActions() {
...
void Game::placeCardEvent(Card _card) {
	...
	if (rules.canPlace(stacktopcard, _card, draw_active) && current_player->hasCard(_card)) {
...
void Game::drawCardEvent() {
	...
	if (draw_active) {
		...
		current_draw_amount = rules.canDraw(current_draw_amount, (*current_player)->getCardsOnHand());
...
void Game::reverseEvent() {
	direction = !direction;
}

void Game::wildEvent(uint32_t _chosencolor) {
	Card invcard = tmpCard; // actual card that is in player's inventory
	Card placecard = Card(_chosencolor, invcard.number); // card that appears on the stack e.g. red wild card
...
void Game::PRINTSTATSPILE() {
	pile.PRINTSTATS();
}

void Game::PRINTSTATSSTACK() {
	stack.PRINTSTATS();
}
...
uint32_t Game::addPlayer(uint32_t puid) {
	...
	if (!active) {
		...
		if (!((players.size() + 1) > rules.getOption("maxPlayers"))) {
			...
			std::shared_ptr<Player> player = std::make_shared<Player>(Player(puid)); // create a player
			uint32_t playerid = player->getID();

			this->players.insert({ playerid, player }); // insert player into this players-map
			return playerid;
		}
		else {
			std::cout << "THE LOBBY IS FREAKING FULL! WHAT THE HELL? ARE YOU GOING TO SQUISH HIM INSIDE OR WHAT?";
			return -1;
		}
	}
	else {
