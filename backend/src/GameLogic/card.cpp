...
bool Card::operator==(Card& _card) {
    if ((this->color == _card.color || this->color == C_NONE) && this->number == _card.number) {
        return true;
    }
    else {
        return false;
...
CardPile::CardPile() {
    ...
    for (int i = 1; i <= 4; i++) {
        // create 1x zero
        createCard(Card(static_cast<e_color>(i), static_cast<e_number>(1)));
        // go through remaining cards(12) 2x
        for (int x = 0; x <= 1; x++) {
            // create cards from 1-9, reverse, skip, draw_two
            for (int j = 2; j <= 13; j++) {
                createCard(Card(static_cast<e_color>(i), static_cast<e_number>(j)));
            }
        }
    }
    // create black cards
    for (int i = 0; i <= 3; i++) {
        createCard(Card(static_cast<e_color>(5), static_cast<e_number>(14)));
        createCard(Card(static_cast<e_color>(5), static_cast<e_number>(15)));
    }
    shuffle();
}
...
void CardPile::createCard(Card _card) {
    cards.push_back(std::make_shared<Card>(_card)); // insert card at last position(top of pile)
    // maxsize++;
    // increaseCurSize();
}
...
void CardPile::destroyCard(Card _card) {
    ...
    for (int i = 0; i < cards.size(); i++) {
        ...
        if ((cards.at(i)->color == _card.color) && (cards.at(i)->number == _card.number)) {
...
std::shared_ptr<Card> CardPile::drawTopCard() {
    ...
    std::shared_ptr<Card> card = cards.at(cards.size() - 1); // gets last vector element
...
void CardPile::addCards(std::vector<std::shared_ptr<Card>> _cards) {
    ...
    /*if (cursize + _cards.size() < maxsize) {
        increaseCurSize(_cards.size()); // increase pilesize by number of cards added
    ...
    cards.insert(cards.end(), _cards.begin(), _cards.end()); // append _cards to cardpile
...
std::vector<std::shared_ptr<Card>> CardStack::drawBackCards() {
    std::vector<std::shared_ptr<Card>> backcards = this->cards; // create copy of all cards on stack
    backcards.pop_back(); // remove last object(top card) from copy

    cards.erase(cards.begin(), cards.end() - 1); // remove all objects except last one(top card) from actual stack
