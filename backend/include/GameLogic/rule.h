...
public:
	...
	/// initialCards - How many cards does each player get to the beginning of the round
	/// </summary>
	
	std::unordered_map<uint8_t, std::shared_ptr<CardRule>> cardrules; // stores all cardrules (cards not listed in here are handled as Regular cards)
	

	/*
	type	-	typeid	-	typename	-	arg1	-	arg2	-	arg3	-	arg4
	typeid	-	1		-	gamerule	-
	typeid	-	2		-	cardrule	-	action	-	cardclr	-	cardnbr	-	mod
	*/
	uint8_t createRule(int _type, uint16_t _arg1, uint16_t _arg2, uint16_t _arg3, uint16_t _arg4); // only used by initial gamecreation

	void activateRule(uint8_t id);
	void deactivateRule(uint8_t id);

	bool canPlace(Card _stacktopcard, Card _playerplacecard, bool _playerDraw); // checks if card can be placed according to rules

	uint16_t canDraw(uint16_t _curDrawCount, uint8_t _cardsOnHand); // returns the max amount the player can draw
	...
	CardRule getCardRule(Card _card); // get Rules corresponding to card according to activated rules
