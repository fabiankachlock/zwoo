...
private:
	...
	uint16_t current_draw_amount; // How much cards have to be drawn at the moment
	const uint32_t ID; // GameID
	Card tmpCard; // used for something like wild events

	CyclingList<std::shared_ptr<Player>>::iterator current_player; // currently active player
	CyclingList<std::shared_ptr<Player>> playerorder;

	std::unordered_map<uint32_t, std::shared_ptr<Player>> players; // all players anticipating in the game
	std::vector<e_gaction> expected_actions; // which order/orders does the game expect to recieve next


	void drawCard(std::shared_ptr<Player> _player); // draws a single card
	void drawCards(std::shared_ptr<Player> _player, uint8_t _amount); // draws multiple cards
	void placeCard(std::shared_ptr<Player> _player, Card _card); // places a card
	void placeCard(std::shared_ptr<Player> _player, Card _card1, Card _card2); // places _card2 onto the stack but removes _card1 from inventory
