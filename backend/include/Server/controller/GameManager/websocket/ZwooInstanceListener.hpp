...
class ZwooInstanceListener
    : public oatpp::websocket::ConnectionHandler::SocketInstanceListener
{
  public:
    ZwooInstanceListener( std::shared_ptr<Logger> _log,
                          std::shared_ptr<ZRPConnector> connector )
        : logger( _log ), connector( connector )
    {
    }

    void
    onAfterCreate( const oatpp::websocket::WebSocket &socket,
