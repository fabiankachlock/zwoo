...
<template>
  <div class="flex justify-between items-center my-1 bg-surface rounded border-border border">
    <p class="pl-2">
      {{ profile.group === GameProfileGroup.System ? t(profile.name) : profile.name }}
    </p>
    <div class="flex">
      <button class="rounded m-1 bg-alt hover:bg-alt-hover text-text flex px-2 py-0.5 items-center group" @click="emit('apply')">
        {{ t('rules.applyProfile') }}
      </button>
      <button v-if="profile.group === GameProfileGroup.User" class="rounded m-1 bg-alt hover:bg-alt-hover text-text" @click="emit('delete')">
